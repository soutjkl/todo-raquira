...
export default function ProductCard({ product, productSelection, setModalWindowState, modalWindowState }) {
    return (
        ...
        <div className="card shadow m-3 position-relative" style={{ width: '15rem' }} onClick={()=>{setModalWindowState(!modalWindowState); productSelection(product)}}>
            <img src={product.imagen_producto} className="card-img-top p-3 pt-4 m-0" alt="..." />
            <div className="card-body pt-0">
                <h5 className="card-title m-0" id='text'>{product.nombre_producto}</h5>
                <p className="card-text m-0" id='price' >${product.precio_unitario.toLocaleString('es-CO')}</p>
            </div>
            {product.estado === 'N' &&
